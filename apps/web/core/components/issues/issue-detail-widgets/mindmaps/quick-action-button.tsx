...
export const IssueMindmapsActionButton = observer(function IssueMindmapsActionButton(props: Props) {
  const { customButton, disabled = false, issueServiceType } = props;
  const { toggleMindmapModal } = useIssueDetail(issueServiceType);

  const handleOnClick = (e: React.MouseEvent<any>) => {
    e.preventDefault();
    e.stopPropagation();
    if (!disabled) {
      toggleMindmapModal(true);
    }
  };
