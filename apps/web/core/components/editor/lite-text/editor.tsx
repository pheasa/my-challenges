...
import type { ITeamWork, MakeOptional } from "@plane/types";
...
export const LiteTextEditor = React.forwardRef(function LiteTextEditor(
  ...
  // fetch team works so the @ mention lists team work entries
  const { isLoading: isTeamWorksLoading } = useSWR(
  ...
  const { fetchMentions } = useEditorMention({
    searchEntity: async (payload) => {
      // ensure team works are loaded before filtering so the first search
      // (right after the editor mounts) never returns an empty list
      if (isTeamWorksLoading) {
        await fetchTeamWorks(workspaceSlug);
      }
      const query = payload.query?.toLowerCase() ?? "";
      const teamWorks = (teamWorkIds ?? [])
        .map((teamWorkId) => getTeamWorkDetails(teamWorkId))
        .filter((teamWork): teamWork is ITeamWork => teamWork !== null)
        .filter((teamWork) => `${teamWork.name} ${teamWork.role ?? ""}`.toLowerCase().includes(query))
          ...
          <LiteTextEditorWithRef
            ...
            mentionHandler={{
              ...
              getMentionedEntityDetails: (id) => ({
                display_name: getTeamWorkDetails(id)?.name ?? getUserDetails(id)?.display_name ?? "",
              }),
            }}
