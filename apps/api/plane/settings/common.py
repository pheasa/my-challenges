...
# Minimal-stack support: when no REDIS_URL is configured (no redis container in
# the compose file), fall back to the in-process LocMemCache so caching and DRF
# throttling keep working without an external store.
if REDIS_URL:
    if REDIS_SSL:
        CACHES = {
            "default": {
...
# Minimal-stack support: when no message broker (rabbitmq) is available, run
# celery tasks eagerly in-process so every `.delay()` call still executes.
# Controlled by env (docker-compose.yml sets CELERY_TASK_ALWAYS_EAGER=1).
