...
class ProjectDetailSerializer(BaseSerializer):
    ...
    team_work_lead = serializers.SerializerMethodField()
    is_favorite = serializers.BooleanField(read_only=True)
    sort_order = serializers.FloatField(read_only=True)
    member_role = serializers.IntegerField(read_only=True)
    anchor = serializers.CharField(read_only=True)

    def get_team_work_lead(self, obj):
        if obj.team_work_lead:
            return {
                "id": obj.team_work_lead.id,
                "name": obj.team_work_lead.name,
                "role": obj.team_work_lead.role,
                "avatar": obj.team_work_lead.avatar,
            }
        return None
