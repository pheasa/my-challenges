        ...
        try:
            ...
            if serializer.is_valid():
                with transaction.atomic():
                    ...
                    # Auto-set the project lead to the creator
                    serializer.instance.lead = request.user
                    serializer.instance.save(update_fields=["lead"])
...
class ProjectDetailAPIEndpoint(BaseAPIView):
    ...
    def get_queryset(self):
        return (
            ...
            .select_related("workspace", "workspace__owner", "default_assignee", "lead")
            .annotate(
                is_member=Exists(
                    ProjectMember.objects.filter(
                        member=self.request.user,
